...
class Send(MessagingHandler):
    ...
    def on_sendable(self, event):
        while event.sender.credit and self.sent < self.total:
            ...
            if (self.sent % 2 == 0):
                msg = Message(id=(self.sent+1),body=open('solace/python_cli/users.avro','rb').read(),durable=self.message_durability)
            else:
...
try:
    ...
    Container(Send(opts['url'], opts['address'], opts['messages'], opts['username'], opts['password'], 2)).run()
